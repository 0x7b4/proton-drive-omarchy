...
Panel {
  id: root
  moduleName: "io.github.yourname.proton-drive"
  manageIpc: false
  ...
  property string uploadDir: Qt.resolvedUrl(
    StandardPaths.writableLocation(StandardPaths.HomeLocation) + "/ProtonDrive/upload"
  )
  property string downloadDir: Qt.resolvedUrl(
  ...
  Process {
    ...
    command: ["sh", "-c",
      ...
      "proton-drive filesystem upload '" + root.uploadDir + "'/* '" +
      root.remotePath + "' --conflict-strategy skip"
    ]
    onExited: function(exitCode) {
      root.busy = false
      root.lastActionMessage = exitCode === 0
