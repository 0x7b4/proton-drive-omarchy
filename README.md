...
```sh
omarchy plugin add https://github.com/yourname/proton-drive-omarchy.git --enable
```
...
cp -r plugin ~/.config/omarchy/plugins/io.github.yourname.proton-drive
omarchy plugin validate ~/.config/omarchy/plugins/io.github.yourname.proton-drive
...
```sh
omarchy bar move io.github.yourname.proton-drive --section right
```

## Désinstaller

```sh
omarchy plugin remove io.github.yourname.proton-drive
```

## Notes / limites connues

- Seules les sous-commandes confirmées de `proton-drive` sont utilisées ici
